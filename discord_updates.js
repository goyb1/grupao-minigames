...
function createStore({db,dataDir}){
 ...
 return {
  ...
  async latestSent(){await init();return db?(await db.query({text: "SELECT data FROM discord_updates WHERE data->>'status'='sent' ORDER BY data->>'updatedAt' DESC LIMIT 1",query_timeout:4000})).rows[0]?.data||null:Object.values(read()).filter(r=>r.status==='sent').sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt))[0]||null;},
