...
function createBackupTools({validateSheet,normalize,attributes,styles}){
 ...
 function campaign(v,owner){
  ...
  const trash=require('./rpg_trash');trash.check(v.sheetTrash||[]);
  result.sheetTrash=(v.sheetTrash||[]).map(e=>{object(e);object(e.member);const target=text(e.target,90);if(target.startsWith('npc:')?!/^npc:[a-zA-Z0-9_-]{1,80}$/.test(target):!Object.hasOwn(result.members,target))fail('Destino inválido na lixeira.');if(!Number.isFinite(Date.parse(e.deletedAt)))fail('Data inválida na lixeira.');const nick=text(e.member.nick,80);if(!target.startsWith('npc:')&&normalize(nick)!==target)fail('Participante inválido na lixeira.');return {id:text(e.id,80),target,deletedAt:text(e.deletedAt,80),member:{nick,sheet:sheet(e.member.sheet),rolls:e.member.rolls??null}};});
  if(new Set(result.sheetTrash.map(e=>e.id)).size!==result.sheetTrash.length)fail('Itens duplicados na lixeira.');trash.check(result.sheetTrash);
