...
(() => {
 ...
 function mount(pitch){
  ...
  function resize(){const width=Math.max(1,viewport.clientWidth-48),height=width*68/105;camera.resize(width,height);pitch.style.width=width+'px';pitch.style.height=height+'px';viewport.style.height=(height+64)+'px';paint();}
  ...
  return {destroy(){resetPointers();observer.disconnect();win.removeEventListener('blur',resetPointers);for(const [name,fn] of listeners)viewport.removeEventListener(name,fn,true);toolbar.remove();viewport.classList.remove('match-camera','is-navigating');viewport.style.height='';viewport.removeAttribute('tabindex');pitch.style.transform='';pitch.style.width='';pitch.style.height='';}};
 }
