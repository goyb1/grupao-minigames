...
(() => {
 ...
 function mount(pitch){
  ...
  function resize(){const full=host.classList.contains('match-fullscreen'),available=Math.max(100,host.clientHeight-toolbar.offsetHeight-(host.querySelector('.match-ruler-toolbar')?.offsetHeight||0)-(host.querySelector('.match-pin-hint')?.offsetHeight||0)-(host.querySelector('.match-quick-tools')?.offsetHeight||0)-140);const width=Math.max(1,full?Math.min(host.clientWidth-48,available*105/68):viewport.clientWidth-48),height=width*68/105;camera.resize(width,height);pitch.style.width=width+'px';pitch.style.height=height+'px';viewport.style.height=(height+64)+'px';paint();}
