...
(() => {
 ...
 function mount(pitch){
  const viewport=pitch.parentElement,doc=pitch.ownerDocument,win=doc.defaultView,camera=createCamera(),points=new Map();let navigating=false,baseline=null,originX=24,originY=24;
  ...
  function fullState(){const active=doc.fullscreenElement===host||fallback;host.classList.toggle('match-fullscreen',active);win.GrupaoPresence?.mount(active?host:null);fullButton.textContent=active?'SAIR DA TELA CHEIA':'TELA CHEIA';fullButton.setAttribute('aria-pressed',String(active));resize();}
  ...
  function resize(){
   const full=host.classList.contains('match-fullscreen');
   if(full)viewport.style.height='';
   const width=Math.max(1,full?Math.min(viewport.clientWidth-48,Math.max(1,viewport.clientHeight-64)*105/68):viewport.clientWidth-48),height=width*68/105;
   if(!full)viewport.style.height=(height+64)+'px';
   originX=full?(viewport.clientWidth-width)/2:24;originY=full?(viewport.clientHeight-height)/2:24;
   camera.resize(width,height);pitch.style.left=originX+'px';pitch.style.top=originY+'px';pitch.style.width=width+'px';pitch.style.height=height+'px';paint();
  }
  ...
  function move(e){if(!navigating||!points.has(e.pointerId))return;stop(e);points.set(e.pointerId,{x:e.clientX,y:e.clientY});const next=geometry(),rect=viewport.getBoundingClientRect();if(baseline){if(next.d>0&&baseline.d>0)camera.zoom(camera.state.scale*next.d/baseline.d,baseline.x-rect.left-originX,baseline.y-rect.top-originY);camera.pan(next.x-baseline.x,next.y-baseline.y);}baseline=next;paint();}
  ...
  function wheel(e){if(!navigating)return;stop(e);const r=viewport.getBoundingClientRect();camera.zoom(camera.state.scale*Math.exp(-clamp(e.deltaY,-100,100)*.003),e.clientX-r.left-originX,e.clientY-r.top-originY);paint();}
  ...
  return {destroy(){win.GrupaoPresence?.mount(null);doc.removeEventListener('fullscreenchange',fullState);doc.removeEventListener('keydown',escapeFull);if(fallback){fallback=false;doc.body.style.overflow=oldOverflow;}if(doc.fullscreenElement===host)doc.exitFullscreen().catch(()=>{});host.classList.remove('match-fullscreen');resetPointers();observer.disconnect();win.removeEventListener('blur',resetPointers);for(const [name,fn] of listeners)viewport.removeEventListener(name,fn,true);toolbar.remove();viewport.classList.remove('match-camera','is-navigating');viewport.style.height='';viewport.removeAttribute('tabindex');pitch.style.left='';pitch.style.top='';pitch.style.transform='';pitch.style.width='';pitch.style.height='';}};
 }
