...
function validateSheet(input,member,master){
 ...
 if(!Number.isInteger(selected)||!member.rolls?.[selected])fail('Escolha um dos dois conjuntos de dados.');
 if([...Object.values(base)].sort((a,b)=>a-b).join(',')!==[...member.rolls[selected]].sort((a,b)=>a-b).join(','))fail('Distribua exatamente os onze dados do conjunto escolhido.');
 ...
 const stats=Object.fromEntries(names.map(a=>{const value=Math.max(1,Math.min(30,base[a]+(bonuses[a]||0)+growth[a]));return[a,{base:base[a],bonus:bonuses[a]||0,growth:growth[a],value,modifier:Math.floor((value-8)/2)}];}));
 return {name:clean(input.name,60,true),age,height,nationality:clean(input.nationality,60,true),position,style,ego:input.ego,selected,base,growth,level,stats,talents:clean(input.talents,1500),weapon:clean(input.weapon,1500),notes:clean(input.notes,3000),photo:photo(input.photo),approved:master?!!input.approved:false};
}
function createRpg({app,express,db,auth,normalize,dataDir}){
 ...
 async function init(){if(db)await db.query('CREATE TABLE IF NOT EXISTS rpg_campaigns (id TEXT PRIMARY KEY, data JSONB NOT NULL)');else{try{local=JSON.parse(fs.readFileSync(file,'utf8'));}catch(e){if(e.code!=='ENOENT')throw e;}}}
 ...
 function view(c,k){member(c,k);return {...c,members:Object.fromEntries(Object.entries(c.members).map(([id,m])=>[id,{...m,rolls:id===k||c.owner===k?m.rolls:undefined}]))};}
 ...
 router.post('/campaigns/:id/roll',route(async req=>{const k=key(req);const c=await mutate(req.params.id,c=>{const m=member(c,k);if(!m.rolls)m.rolls=Array.from({length:2},()=>Array.from({length:11},()=>crypto.randomInt(1,13)));});return view(c,k);}));
 router.put('/campaigns/:id/sheets/:who',route(async req=>{const k=key(req),target=req.params.who;const c=await mutate(req.params.id,c=>{member(c,k);const m=member(c,target),master=c.owner===k;if(!master&&target!==k)fail('Você só pode editar sua ficha.',403);if(!master&&m.sheet?.approved)fail('Peça ao mestre para liberar a edição da ficha.',403);m.sheet=validateSheet(req.body,m,master);});return view(c,k);}));
